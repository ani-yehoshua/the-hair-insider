...
export const MAX_SCORES = Object.fromEntries(
    DIMENSIONS.map((dim) => [
        ...
        QUESTIONS.reduce(
            (sum, q) =>
                sum + Math.max(0, ...q.options.map((o) => o.scores[dim] || 0)),
            0,
        ),
    ]),
...
export function calculateResults(answers: AnswerMap) {
    ...
    const evidenceCount = {} as Record<Dimension, number>;
    const evidenceByDimension = {} as Record<Dimension, string[]>;
    DIMENSIONS.forEach((d) => {
        scores[d] = 0;
        evidenceCount[d] = 0;
        evidenceByDimension[d] = [];
    });
    ...
    for (const q of QUESTIONS) {
        ...
        for (const [dimStr, val] of Object.entries(option.scores)) {
            ...
            if (
                val > 0 &&
                !evidenceByDimension[dim].includes(option.label.toLowerCase())
            ) {
        ...
        if (!structuralQIds.includes(qId)) {
            ...
            if (!isExcluded && label.length > 4) {
                if (isDamage) {
                    if (damageObservations.length < 4)
                        damageObservations.push(label);
                } else {
                    if (healthyObservations.length < 4)
                        healthyObservations.push(label);
                }
            }
        }
    }

    const getNorm = (dim: Dimension) =>
        MAX_SCORES[dim] > 0 ? scores[dim] / MAX_SCORES[dim] : 0;
    ...
    const corroboratingStrengthSignal =
        ...
        answers.chemical === 3 ||
        answers.chemical === 4;
    const proteinSupportConfirmed =
        positiveProteinResponse && corroboratingStrengthSignal;

    let primaryCause = "";
    let isHealthyFallback = false;
    let behaviorToStop = "over-handling your hair";

    const validCauses: { dim: Diagnosis; norm: number; name: string }[] = [];

    if (mNorm >= 0.35 && evidenceCount["M"] >= 2)
        validCauses.push({
            dim: "M",
            norm: mNorm,
            name: "dryness / low flexibility",
        });
    if ((pNorm >= 0.35 && evidenceCount["P"] >= 2) || proteinSupportConfirmed) {
        validCauses.push({
            dim: "P",
            norm: Math.max(pNorm, 0.4),
            name: "compromised internal strength",
        });
    }
    if (buNorm >= 0.35 && evidenceCount["BU"] >= 2)
        validCauses.push({ dim: "BU", norm: buNorm, name: "product buildup" });
    if (getNorm("LP") >= 0.35 && evidenceCount["LP"] >= 2)
        validCauses.push({
            dim: "LP",
            norm: getNorm("LP"),
            name: "low-porosity resistance",
        });
    if (getNorm("HP") >= 0.35 && evidenceCount["HP"] >= 2)
        validCauses.push({
            dim: "HP",
            norm: getNorm("HP"),
            name: "high-porosity moisture loss",
        });
    if (getNorm("EL") >= 0.35 && evidenceCount["EL"] >= 2)
        validCauses.push({
            dim: "EL",
            norm: getNorm("EL"),
            name: "elasticity loss",
        });
    if (hcNorm >= 0.35 && evidenceCount["HC"] >= 2)
        validCauses.push({
            dim: "HC",
            norm: hcNorm,
            name: "heat / chemical stress",
        });
    if (meNorm >= 0.35 && evidenceCount["ME"] >= 2)
        validCauses.push({
            dim: "ME",
            norm: meNorm,
            name: "mechanical tension",
        });
    if (brNorm >= 0.35 && evidenceCount["BR"] >= 2)
        validCauses.push({ dim: "BR", norm: brNorm, name: "breakage" });
    if (shNorm >= 0.35 && evidenceCount["SH"] >= 2)
        validCauses.push({ dim: "SH", norm: shNorm, name: "shedding" });
    if (scNorm >= 0.35 && evidenceCount["SC"] >= 2)
        validCauses.push({ dim: "SC", norm: scNorm, name: "scalp imbalance" });
    ...
    const confirmedBuildup =
        (answers.residue === 1 || answers.residue === 2) &&
        answers.clarify === 0;
    if (confirmedBuildup) {
    ...
    const sourceIndex = validCauses.findIndex(
        (c) => (c.dim === "ME" || c.dim === "HC") && c.norm >= 0.4,
    );
    ...
    if (primaryCause) {
        const winningDimensions: Dimension[] =
            diagnosis === "MP"
                ? ["M", "P"]
                : diagnosis === "POROSITY_MIXED"
                  ? ["LP", "HP"]
                  : [diagnosis as Dimension];
        finalObservations = winningDimensions
            .flatMap((dim) => evidenceByDimension[dim])
        ...
        answers.pattern === 1
            ...
            : answers.pattern === 2
              ...
              : answers.pattern === 3
                ...
                : "straight";
    const density =
        answers.density === 0 || answers.density === 3
            ? "low"
            : answers.density === 2
              ? "high"
              : "medium";
    const colour =
        answers.chemical === 2
            ...
            : answers.chemical === 1 ||
              ...
              ? "colour"
              : "natural";
    const stylePreference: HairProfile["stylePreference"] =
        answers.usualStyle === 1
            ? "blowout"
            : answers.usualStyle === 2
              ? "straightened"
              : answers.usualStyle === 3
                ? "curled"
                : answers.usualStyle === 4
                  ? "protective"
                  : "natural";
    const washFrequency: HairProfile["washFrequency"] =
        answers.shampooFrequency === 0
            ? "daily"
    ...
    const profile = {
        ...
        strengthNeed:
            positiveProteinResponse ||
            bothMP ||
            (pNorm >= 0.35 && evidenceCount["P"] >= 2),
        ...
        irritatedScalp:
            scNorm >= 0.4 || answers.scalp === 3 || answers.scalp === 4,
    } as const;
    const paidRoutine = buildPaidRoutine(profile);
    const [product1, product2, product3] = selectFoundationRecommendations(
        profile,
        paidRoutine,
    );
    ...
    if (positiveProteinResponse) {
        const proteinTiming = paidRoutine.find(
    ...
    if (hcNorm >= 0.4)
        behaviorToStop =
            "frequent high-heat styling or overlapping chemical processes";
    else if (meNorm >= 0.4)
        behaviorToStop = "tight hairstyles and excessive tension";
    else if (buNorm >= 0.4)
        behaviorToStop = "layering heavy products without regular clarifying";
    else if (brNorm >= 0.4) behaviorToStop = "rough detangling on dry ends";
    else if (isHealthyFallback)
        behaviorToStop =
            "inconsistent routines and unnecessary mechanical friction";
